...
st.set_page_config(layout="wide", page_title="JOA DATA SAVIOR V214")
...
# --- [V214: 강력한 데이터 보존 및 순위 추격 엔진] ---
def run_savior_scan_v214(skw, spmid, scmid, snote):
    t_pmid = str(spmid).strip()
    db = get_db()
    
    # 🔍 [방패] 장부에서 이 P-MID로 저장된 가장 완벽한(긴 이름, 높은 수치) 데이터를 먼저 꺼냅니다.
    best = db.execute("""
    ...
    with st.spinner(f"🛡️ '{skw}' 데이터 정밀 복구 및 순위 추격 중..."):
        # 공식 API 스캔 (순위용)
    ...
    # --- [SMART JUDGE: 사장님이 원하시는 그 로직] ---
    # 이름: 네이버에서 준 게 기존보다 짧으면 무조건 무시!
    ...
    # 수치: 0 들어오면 장부에 0이라고 적지 않고 기존 수치 유지!
...
with st.container():
    ...
    if c5.button("🚀 정밀 조회/복구", key="main_btn"):
        if nk and np: run_savior_scan_v214(nk, np, nc, nn)
...
for idx, (kw, mid, m_val, _) in enumerate(items):
    # 화면 표시용 데이터도 장부 내 최고 기록을 실시간으로 가져옵니다.
    best = db.execute("""
        SELECT name, reviews, purchase FROM logs 
        WHERE p_mid=? 
        ORDER BY length(name) DESC, CAST(purchase AS INTEGER) DESC LIMIT 1
    ...
    with st.container():
        ...
        with col3:
            st.markdown(f"### 🔍 {kw}")
            st.markdown(f"**{title}**") # 역대 최고 이름 고정
            if m_val: st.info(f"📝 {m_val}")
            st.caption(f"MID: {mid} | C: {curr[1]}")
        with col4:
            st.write(f"구매 **{best[2]}**") # 역대 최고 구매수 고정
            st.write(f"리뷰 **{best[1]}**") # 역대 최고 리뷰수 고정
        with col5:
            ...
            if st.button("🔄 순환업데이트", key=f"btn_{mid}_{idx}"):
                run_savior_scan_v214(kw, mid, curr[1], m_val)
