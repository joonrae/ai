...
# ==========================================
# VERSION: V186-IMAGE-RECOVERY (이미지 완벽 박제)
# ==========================================
...
st.set_page_config(layout="wide", page_title="JOA CLOUD SNIPER V186")
...
if not st.session_state.auth:
    st.markdown("<h1 style='text-align:center; margin-top:50px;'>🛡️ JOA CLOUD SNIPER V186</h1>", unsafe_allow_html=True)
...
def run_scan_v186(skw, spmid, scmid, snote, user_id):
    ...
    # 2. [특별 로직] !fix (이름/데이터), !img (이미지) 주입
    if snote.startswith("!fix"):
        m_name = snote.replace("!fix", "").strip()
        db.execute("INSERT OR REPLACE INTO product_meta (p_mid, name, img, reviews, purchase) VALUES (?,?,?,?,?)", (t_pmid, m_name, m_img, m_rev, m_pur))
    if "!img" in snote:
        m_img = snote.split("!img")[1].strip().split(" ")[0]
        db.execute("INSERT OR REPLACE INTO product_meta (p_mid, name, img, reviews, purchase) VALUES (?,?,?,?,?)", (t_pmid, m_name, m_img, m_rev, m_pur))
    db.commit()

    with st.spinner(f"🚀 '{skw}' 정밀 수색 및 이미지 복구 중..."):
        f_rank, off_name, off_img = get_main_rank(skw, t_pmid, t_cmid)
    ...
    if spy and int(spy['pur']) > 0:
        ...
        final_img = spy['img'] if spy['img'] else (off_img if off_img else m_img)
        final_rev, final_pur, final_inner = str(spy['rev']), str(spy['pur']), spy['inner']
        db.execute("INSERT OR REPLACE INTO product_meta VALUES (?,?,?,?,?)", (t_pmid, final_name, final_img, final_rev, final_pur))
    else:
        # 실패 시 금고 유지 (단, 공식API가 더 긴 이름을 주면 업데이트)
        final_name = off_name if len(off_name) > len(m_name) else m_name
        final_img = m_img if m_img else off_img
        final_rev, final_pur, final_inner = m_rev, m_pur, 0
...
st.title(f"🚀 JOA CLOUD SNIPER V186 - {st.session_state.user}님")
...
with st.container():
    ...
    if c5.button("🚀 추격 시작", use_container_width=True):
        if nk and np: run_scan_v186(nk, np, nc, nn, st.session_state.user)
...
for idx, (kw, mid, m_val, _) in enumerate(items):
    ...
    with st.container():
        ...
        with col2:
            # 이미지 출력 강화
            if img and img.startswith("http"): st.image(img, width=140)
            else: st.info("🖼️ 이미지 찾는 중")
        with col3:
            st.markdown(f"### 🔍 {kw}")
        ...
        with col4:
            ...
            for d in range(8):
                ...
                h_html += f"<div style='flex:1; border:1px solid #e2e8f0; padding:6px; text-align:center; background:#f8f9fa; border-radius:8px;'><div style='font-size:0.6rem; color:#a0aec0;'>{t[5:]}</div><div style='font-size:0.8rem; font-weight:bold;'>{m_rk}</div></div>"
        ...
        with col6:
            ...
            bc = st.columns(2)
            if bc[0].button("🔄", key=f"r_{row[0]}"): run_scan_v186(kw, mid, cmid, memo_text, st.session_state.user)
            if bc[1].button("🗑️", key=f"d_{row[0]}"):
                db_del = get_db(); db_del.execute("DELETE FROM logs WHERE keyword=? AND p_mid=? AND note=? AND user_id=?", (kw, mid, memo_text, st.session_state.user)); db_del.commit(); db_del.close(); st.rerun()
